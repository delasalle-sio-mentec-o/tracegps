...
// 	}
	...
	public static function estUneDateValide2 ($laDateAvalider)
	{
	    $pos1 = strpos($laDateAvalider, '-');
	    $pos2 = strrpos($laDateAvalider, '-', $pos1 + 1);
	    
	    $pos3 = strpos($laDateAvalider, '/');
	    $pos4 = strrpos($laDateAvalider, '/', $pos3 + 1);
	    
	    $longueur = strlen($laDateAvalider);
	    if(($pos1 != 0 && $pos2 != 0 && $longueur == 10) || ($pos3 != 0 && $pos4 != 0 && $longueur == 10))
	    {
	        $chaine1 = substr ($laDateAvalider, 0, 2);
	        $chaine2 = substr ($laDateAvalider, 3, 2);
	        $chaine3 = substr ($laDateAvalider, 6);
	        if (is_numeric($chaine3) == false)
	            return false;
	            $jour = (int)($chaine1);
	            $mois = (int)($chaine2);
	            $an = (int)($chaine3);
	            $check = checkdate($mois, $jour, $an);
	            return $check;
	    }
	    else
	    {
